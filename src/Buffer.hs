...
moveForwardToDelimiter c (B n t p m pg) = case R.elemIndexForward t p c of
...
moveBackwardToDelimiter c (B n t p m pg) = case R.elemIndexBackward t p c of
...
goToLine n (B nm t _ m pg) = B nm t np m pg where
